We report on the application of our approach for surveillance synthesis in two case studies. We have implemented the proposed method  in \texttt{Python}, using the \texttt{slugs} reactive synthesis tool~\cite{EhlersR16}. The experiments were performed on an Intel i5-5300U 2.30 GHz CPU with 8 GB of RAM. 

%In both cases, the target is being controlled by a human while the agent responds using the synthesized surveillance strategy.
