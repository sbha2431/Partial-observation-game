...
Intuitively, the abstraction of a set $L$ of locations of the target is set of elements of $\part$ that cover $L$, and each of them has non-empty intersection with $L$. The concretization of a set of elements of $\part$ is the set of locations of the target formed by the union of these sets. Thus, we have $\gamma(\alpha_{\part}(L))\supseteq L$, which means that the belief of the agent is overapproximated by the abstraction as desired.
